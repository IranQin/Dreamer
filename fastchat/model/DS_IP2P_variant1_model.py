...
from fastchat.diffusers0202_unet.unet_2d_condition import UNet2DConditionModel
...
class InstructPix2Pix_model(nn.Module):
    ...
    def init_sd_vae_unet(self, is_position_embeddings, diffusion_loss_weight):
        ...
        if is_position_embeddings == True:
            # Diffusers-0.20.2 unet
            # <class 'fastchat.diffusers0202_unet.unet_2d_condition.UNet2DConditionModel'>
            from fastchat.diffusers0202_unet.unet_2d_condition import UNet2DConditionModel
    ...
    def forward(
            self,
            # 这些变量都是从data_module返回出来的，后面的变量都是空的
        ...
        # CFG-1. Sample masks for the edit prompts -> 10%的概率把文本condition置空
        ...
        # Final text conditioning -> 把prompt_mask是True的地方换成null-text ""
        ...
        # 2. Diffusion loss: Convert images to latent space -> 这里就是diffusers经典的stable diffusion loss
        ...
        """ 注意，这里的需要把original image当成condition帮助edited去噪 -> InstructPix2Pix对齐 """
        ...
        # CFG-2. Sample masks for the original images -> 5%-15%的概率把image condition置空
        ...
        # Concatenate the original_image_embeds with the noisy_latents -> 注意，noisy_latents需要放在前面，因为前四个通道的unet.conv_in是SD的权重

...
from fastchat.conversation_v01 import get_conv_template
...
class Minecraft_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        # Minecraft_Dataset_path = '/mnt/petrelfs/qinyiran/SmartEdit_231226/dataset/mllm_diffusion_dataset/start_frame_num_1_only_mine_kill_first_10'+'/index.json'
...
# 5. Navigation_Dataset
class Navigation_Dataset(Dataset):
    '''
    according to Navigation, the dataset can be used to train models to follow edit instructions.
    Edit instructions are available in the 'instruction'. 'source_img' can be used with the 'instruction' and 'target_img' denotes the image after applying the 'instruction' on the 'source_img'.
    "source_img" + "target_img" + "instruction"
    Dataset({features: ['img_id', 'turn_index', 'source_img', 'mask_img', 'instruction', 'target_img'], num_rows: 8807})
    '''
    def __init__(self,
                 Navigation_Dataset_path,
                 Navigation_Dataset_resolution_for_SD,
                 CLIP_tokenizer):

        # Navigation Dataset path
        self.Navigation_Dataset_path = Navigation_Dataset_path
        js_path = self.Navigation_Dataset_path
        # Navigation_Dataset_path = '/mnt/petrelfs/qinyiran/SmartEdit_231226/dataset/mllm_diffusion_dataset/start_frame_num_1_only_mine_kill_first_10'+'/index.json'
        self.Navigation_Data_List = []

        with open(js_path, 'r') as data_file:
            self.Navigation_Data_List = json.load(data_file)
        #data_list keys: 'image1', 'image2'
        # 256
        self.Navigation_Dataset_resolution_for_SD = Navigation_Dataset_resolution_for_SD
        # SD transformation
        self.Navigation_Dataset_transform = transforms.Compose([transforms.RandomHorizontalFlip(p=0.5),
                                                               transforms.CenterCrop(self.Navigation_Dataset_resolution_for_SD)])
        # CLIP tokenizer
        self.CLIP_tokenizer = CLIP_tokenizer


    def __len__(self,):
        return len(self.Navigation_Data_List)

    def __getitem__(self, index):
        # Loading Path...
        Navigation_Dataset_sample = self.Navigation_Data_List[index]
        # {'source_img': <PIL.Image.Image image mode=RGB size=500x500 at 0x7F327BE01100>, 'target_img': <PIL.Image.Image image mode=RGB size=1024x1024 at 0x7F327BE010D0>, 'instruction': 'let the asparagus be replaced with sausages'}

        # convert into torch style, png RGBA -> RGB
        Navigation_Dataset_source_img = Image.open(Navigation_Dataset_sample['image1'])   
        Navigation_Dataset_source_img = Navigation_Dataset_source_img.convert('RGB')
        Navigation_Dataset_source_img = np.transpose(Navigation_Dataset_source_img, (2, 0, 1)) # (3, 256, 256)
        Navigation_Dataset_target_img = Image.open(Navigation_Dataset_sample['image2'])   
        Navigation_Dataset_target_img = Navigation_Dataset_target_img.convert('RGB')
        Navigation_Dataset_target_img = np.transpose(Navigation_Dataset_target_img, (2, 0, 1)) # (3, 256, 256

        # # 1. Original Image & 2. Edited Image for SD input
        Navigation_Dataset_SD_input = np.concatenate([Navigation_Dataset_source_img, Navigation_Dataset_target_img])
        Navigation_Dataset_SD_input = torch.tensor(Navigation_Dataset_SD_input)
        Navigation_Dataset_SD_input = 2 * (Navigation_Dataset_SD_input / 255) - 1
        # Navigation_Dataset_SD_input = self.Navigation_Dataset_transform(Navigation_Dataset_SD_input)
        Navigation_Dataset_source_img, Navigation_Dataset_target_img = Navigation_Dataset_SD_input.chunk(2)

        # 3. Edited Prompt input_ids(必须叫'input_ids') -> edited text prompt
        # edited_prompt = Navigation_Dataset_sample['instruction']
        edited_prompt = 'Keep looking until you find your target.'
        Navigation_Dataset_instruction = self.CLIP_tokenizer(edited_prompt,
                                                            max_length=self.CLIP_tokenizer.model_max_length,
                                                            padding="max_length",
                                                            truncation=True,
                                                            return_tensors="pt")
        input_ids = Navigation_Dataset_instruction.input_ids[0]

        # Navigation_Dataset dataloader -> 3 parts -> [bs, 3, 256, 256], [bs, 3, 256, 256], ['let the asparagus be replaced with sausages']

        return {'original_img': Navigation_Dataset_source_img,
                'edited_img': Navigation_Dataset_target_img,
                'input_ids': input_ids}

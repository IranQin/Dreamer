...
from fastchat.util.args_nv import ModelArguments, DataArguments, TrainingArguments
from fastchat.util.dataset import Navigation_Dataset

# import which model...
from fastchat.model.InstructPix2Pix_model import InstructPix2Pix_model
...
def train():
    ...
    data_args.NavigationDataset_path = os.path.abspath(data_args.NavigationDataset_path)
    ...
    ####################################################################################
    Navigation_train_dataset = Navigation_Dataset(
        Navigation_Dataset_path=data_args.NavigationDataset_path,
        Navigation_Dataset_resolution_for_SD=data_args.NavigationDataset_resolution_for_SD,
        CLIP_tokenizer=CLIP_tokenizer)

    Navigation_train_dataloader = torch.utils.data.DataLoader(Navigation_train_dataset, batch_size=1, num_workers=8)
    print(Navigation_train_dataset, Navigation_train_dataloader)
    print('Checking Navigation train dataset...', len(Navigation_train_dataset))
    index = 0
    for step, batch_data in enumerate(Navigation_train_dataloader):
    ...
    data_module = dict(train_dataset=Navigation_train_dataset, eval_dataset=None)
